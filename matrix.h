...
void Matrix::setDiagonal(const vector<double>& diagValues) {
        if (diagValues.size() != min(r, c)) {
            throw invalid_argument("Number of diagonal values must match the minimum of rows and columns.");
        }
        for (size_t i = 0; i < diagValues.size(); ++i) {
            mat_data[i * c + i] = diagValues[i];
        }
    }
Matrix diag_matrix(const vector<double>& diagValues) {
    size_t size = diagValues.size();
    Matrix diagonalMatrix(size, size);
    diagonalMatrix.setDiagonal(diagValues);
    return diagonalMatrix;
}
